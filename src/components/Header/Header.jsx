...
const Header = () => {
  ...
  return (
   <header className="fixed top-0 w-full flex items-center justify-between bg-transparent z-50 px-4 sm:px-6 md:px-20 lg:px-40 py-2 sm:py-6">
  {/* Logo */}
  ...
  <div className="hidden sm:flex bg-white/10 backdrop-blur-md rounded-full overflow-hidden relative z-10 border border-gray-400/40 shadow-[0_0_8px_rgba(156,163,175,0.8)] mx-auto">
    {menuItems.map((item) => (
      <a
        key={item}
  ...
  <div className="flex items-center relative z-10 ml-auto">
    ...
    {/* Mobile Menu Button */}
    <button
      className="sm:hidden p-2 ml-3 rounded-full bg-white/10 border border-gray-400/40 backdrop-blur-md text-white"
    ...
    <div className="absolute top-full left-0 right-0 mt-2 w-full bg-white/10 backdrop-blur-md border-t border-gray-400/30 shadow-lg sm:hidden">
      <ul className="flex flex-col divide-y divide-white/10">
        ...
        <li className="flex justify-center px-6 py-8 w-full max-w-2xl">
          <AnimatedButton className="w-full text-xl px-8 py-2">
